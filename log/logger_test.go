...
func TestErrWriter(t *testing.T) {
	...
	log := captureOutput(func() {
		l, err := New(ZapLogger, append(opts, WithWriter(os.Stdout), ErrorOutput(ioutil.Discard))...)
		if err != nil {
			t.Fatal(err)
		}
	...
	log = captureOutput(func() {
		l, err := New(ZapLogger, append(opts, ErrorOutput(os.Stdout), WithWriter(ioutil.Discard))...)
		if err != nil {
			t.Fatal(err)
		}
